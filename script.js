...
// ================================
// 👥 FRIENDS SECTION — Optimized Firebase Version (Single Import)
// ================================
document.addEventListener("DOMContentLoaded", () => {
  const loginRow = document.getElementById("loginRow");
  const friendsSection = document.getElementById("friendsSection");
  ...
  const friendPin = document.getElementById("friendPin");
  const addFriendBtn = document.getElementById("addFriendBtn");
  const friendsList = document.getElementById("friendsList");

  let currentUserCode = null;

  // ✅ Firestore helpers (using the already-imported methods)
  async function getUserDoc(code) {
    return await getDoc(doc(window.db, "users", code));
  }
  async function setUserDoc(code, data) {
    return await setDoc(doc(window.db, "users", code), data);
  }
  async function updateUserDoc(code, data) {
    return await updateDoc(doc(window.db, "users", code), data);
  }
  ...
  loginBtn.addEventListener("click", async () => {
    ...
    try {
      const userSnap = await getUserDoc(code);
      if (userSnap.exists()) {
        ...
        await setUserDoc(code, { email, pin, friends: [] });
      }
      ...
      console.error(err);
      loginErr.textContent = "Error during login.";
      loginErr.style.display = "block";
    }
  });
  ...
  async function renderFriends() {
    ...
    const code = currentUserCode || localStorage.getItem("userCode");
    if (!code) return;

    const snap = await getUserDoc(code);
    ...
    friends.forEach(friend => {
      const li = document.createElement("li");
      li.textContent = `${friend.title || "Friend"} — ${friend.code}`;

      const viewBtn = document.createElement("button");
      viewBtn.textContent = "View";
      viewBtn.addEventListener("click", () => {
        window.location.href = `/list?code=${encodeURIComponent(friend.code)}&pin=${encodeURIComponent(friend.pin)}`;
      });
      ...
      removeBtn.addEventListener("click", async () => {
        ...
        await updateUserDoc(code, { friends: updatedFriends });
        renderFriends();
      });
  ...
  addFriendBtn.addEventListener("click", async () => {
    ...
    const code = friendCode.value.trim().toLowerCase();
    const pin = friendPin.value.trim();
    if (!code || !pin) return;

    const userCode = currentUserCode || localStorage.getItem("userCode");
    if (!userCode) return;

    const snap = await getUserDoc(userCode);
    const friends = snap.data().friends || [];
    friends.push({ title, code, pin });
    await updateUserDoc(userCode, { friends });
